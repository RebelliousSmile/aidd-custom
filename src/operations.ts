    ...
    if (existsSync(cmdSrc)) {
      for (const file of readdirSync(cmdSrc).filter(f => f.endsWith('.md') && /^\d+_/.test(f))) {
        ...
        if (tool === 'copilot') {
          ...
          installed.push(key);
          if (hashes) hashes[key] = hashContent(transformed);
        } else {
          if (write) {
            mkdirSync(destDir, { recursive: true });
...
export function installToolOverlay(
  tool: ToolType,
  projectRoot: string,
  overlayTempDir: string,
  hashes?: Record<string, string>,
  ...
  if (hasFeature(tool, 'agents')) {
    ...
    if (existsSync(srcDir)) {
      if (write) mkdirSync(destDir, { recursive: true });
      const files = readdirSync(srcDir).filter(f => f.endsWith('.md'));
      for (const file of files) {
        const content = readFileSync(join(srcDir, file), 'utf-8');
        const transformed = applyTransform(cfg.transform.agents, content, file);
        if (write) writeFileSync(join(destDir, file), transformed);
...
export function installGlobalOverlay(
  ...
  if (existsSync(cmdsSrc)) {
    for (const file of readdirSync(cmdsSrc).filter(f => f.endsWith('.md') && /^\d+_/.test(f))) {
      ...
      installed.push(key);
      if (hashes) hashes[key] = hashContent(content);
    }
  }
...
export function compareWithOverlay(rootDir: string, overlayTempDir: string, isGlobal = false): OverlayComparison {
  ...
  const overlayUpdated: string[] = [];

  if (!noHashBaseline) {
    for (const file of index?.files ?? []) {
      if (!existsSync(join(rootDir, file))) continue; // already in missingFromDisk
